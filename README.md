...
**11** projects listed · **01** IEEE publication · **100** Lighthouse perf · **0kb** JS dependencies
...
**Editorial devices** — Section headers follow a fixed grammar: a numbered mono label `(01) — Selected Work` above a Syne title where one word slips into *serif italic*. Nav links carry superscript counts (Work <sup>11</sup>), the second hero line renders as **outline text** via `-webkit-text-stroke` with a solid-color `@supports` fallback, and hairlines — 1px at 14% ink — rule every section like a printed spread.
...
- **`curl -L mosambiswas.com/cv`** · the résumé in colour, in a terminal, from a static host. [`cv/index.html`](cv/index.html) is one file that is two things: its HTML head hides inside an OSC escape sequence (`ESC ] … BEL`) that terminals swallow unprinted, so curl shows only the coloured text; a browser reads the same bytes as HTML, and [`cv.js`](cv/cv.js) turns the colour codes into styled spans and fits the 78 columns to the window. The file is generated by [Shellfolio](https://github.com/BiswasMosam/Shellfolio) (`go run . -cv ../BiswasMosam.github.io/cv/index.html`), so regenerate it whenever the résumé changes.
- **↑↑↓↓←→←→BA** · the Konami code puts the ember field into overdrive for three seconds: the smoke churns five times faster and far more of it catches fire (`uOver` in the shader lowers the ember threshold; time is accumulated, not read off the clock, so the speed-up never makes the smoke jump).
- **Bot paths** · `/wp-admin`, `/.env`, `/.git/config`, `/phpmyadmin`, `*.php` and friends get a 404 that answers back, for the rare human who tries one by hand.
- **[`/help`](https://www.mosambiswas.com/help)** · the spoiler page: every egg above, where it is and how to trigger it, with a live try-it for most of them (Konami keycaps that light up as you type, a whisper and a lens to hover, the source portrait fetched on demand, copy buttons for the curl command). `noindex`, out of the sitemap, and linked only from the 404 terminal's `help` and `humans.txt`. **Add every new egg here.**
